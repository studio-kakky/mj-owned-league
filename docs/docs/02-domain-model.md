...
| umaPattern | enum | League の `format` に応じて選べる値が切り替わる。4P 系: `UMA_10_30` / `UMA_10_20` / `UMA_5_10`。3P 系: `UMA_3P_40` / `UMA_3P_30` / `UMA_3P_20` / `UMA_3P_15`。 |
| tobiEnabled | boolean | 飛び賞 ON/OFF |
| tobiPoint | decimal? | 飛び賞のポイント値（`tobiEnabled = true` のとき必須） |

#### ウマパターン（MVP）

4 人麻雀と 3 人麻雀で **別系統のパターン** を持つ。Ruleset の `umaPattern` は、所属 League の `format` が 4P 系か 3P 系かによって選べる値が切り替わる。
...
3 人麻雀は 2 着が中央（0）となる対称パターンで揃える。1 着 `+N` / 2 着 `0` / 3 着 `−N`。
...
- 同点時のタイブレーク順序: 1. トップ回数（多い順）→ 2. 合計素点（高い順）→ 3. 平均着順（小さい順）。
