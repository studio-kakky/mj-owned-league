...
2. URL を開き、招待元情報を確認 → 「Google で承諾」で Google OAuth サインイン → アカウント作成
...
1. Match 詳細ページの対局リストから対象 Game の操作メニュー → 「編集」または「削除」
2. 編集の場合、F6 と同じ「対局を追加」モーダルに現在値が初期値で表示される
3. 保存時にポイント・順位を再計算
4. 削除は **物理削除**（DB からレコードを除去）
